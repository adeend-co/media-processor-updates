...
# ==============================================================================
# [MPI 4.3 核心系統 - 最終純淨版]
# 特性：無固定參數 / 無人為假設 / 線性代數動態約束
# 取代舊版：BayesianInferenceEngine, DualTrackManager, 及所有舊版評估邏輯
# ==============================================================================
...
# 修正說明 V2: 
# 1. 維持 Z-Score 標準化 (解決單位問題)
# 2. 新增「零變異數陷阱」防護 (解決 N<2 時 Naive 模型過度自信導致的 InfoGain 爆炸)
# ==============================================================================
class PerformanceEvaluator:
    def evaluate(self, y_true, y_pred, y_std, dof):
        ...
        # --- 步驟 0: 數據標準化 ---
        # 讓評分機制具備「尺度不變性 (Scale Invariance)」
        scaler_mean = np.mean(y_true)
        scaler_std = np.std(y_true)
        if scaler_std < 1e-9: scaler_std = 1.0 
        
        # 轉為 Z-Score 空間 (無單位)
        target_y = (y_true[1:] - scaler_mean) / scaler_std
        naive_mu = (y_true[:-1] - scaler_mean) / scaler_std
        model_mu = (y_pred[1:] - scaler_mean) / scaler_std
        
        # 標準差縮放，並設定物理下限
        # MIN_STD_Z = 0.01 代表在標準化空間中，我們不相信任何預測能精準到小於 1% 的全域波動
        # 這能有效防止 logpdf 產生天文數字
        MIN_STD_Z = 0.01
        model_sigma = np.clip(y_std[1:] / scaler_std, MIN_STD_Z, None)
        ...
        # --- 步驟 1: 建立穩健的 Naive 基準 ---
        # 修正: 當歷史資料不足(N<2)或數值完全相同(Std=0)時，
        # Naive 不應假設 Std=0 (無限自信)，而應退回全域標準差 (Std=1.0)
        naive_std_list = []
        # 使用 expanding window 計算歷史波動
        for i in range(len(target_y)):
            # 取直到當下的歷史數據 (對應 target_y[:i+1] 的原始序列部分)
            history_segment = target_y[:i+1]
            if len(history_segment) < 2:
                # 只有 1 筆資料，無法計算標準差 -> 假設為全域標準差 (1.0)
                current_std = 1.0
            else:
                current_std = np.std(history_segment, ddof=1)
                # 如果計算出來是 0 (數據完全沒變)，也強平為 1.0，避免無限懲罰
                if current_std < MIN_STD_Z:
                    current_std = 1.0
            
            naive_std_list.append(current_std)
            
        naive_std = np.array(naive_std_list)
        ...
        # 增益值 (Nats)
        # 由於加上了 MIN_STD_Z 保護，這個數值現在會被限制在合理區間 (例如 -5 到 +5 之間)
...
def execute_bayesian_validation(df, target_col, freq_prediction, colors):
    ...
    print(f"  ● 貝氏學派 : {bayes_val:,.0f} (95% CI: ±{t_crit*bayes_sigma:,.0f})")
    ...
    if prequential_results is not None:
        ...
        if errors is not None and len(errors) > window_size:
            ...
            for t in range(window_size, len(errors)):
                past_errors = np.abs(errors[t-window_size : t]) 
                current_error = np.abs(errors[t])               
...
def run_robust_decomp_forecaster(monthly_expenses_df, steps_ahead, colors=None, verbose=True):
    ...
    return (final_prediction_seq, historical_pred, None, None, lower_seq, upper_seq, None)
...
def run_full_ensemble_pipeline(monthly_expenses_df, steps_ahead, colors, verbose=True):
    ...
    pwa_weights = 1 / (np.array(maes) + 1e-9)
    pwa_weights /= np.sum(pwa_weights)
...
def analyze_and_predict(file_paths_str: str, no_color: bool):
    ...
    if analysis_data is not None and len(analysis_data) >= 2:
        ...
        if num_months >= 36:
            ...
            # [移除] 舊版 CV MPI 計算: _, cv_mpi_scores = run_dynamic_strategy_ensemble_cv(...)
        ...
        if future_pred_seq is not None:
            ...
            # [關鍵更新] 雙軌貝氏驗證 + MPI 4.3 核心評估
            # 這裡會觸發新的 BMA 計算、RLF 融合與 MPI 4.3 報告輸出
            if monthly_expenses is not None and not monthly_expenses.empty:
        ...
        if historical_pred is not None:
            ...
            # [移除] 舊版 MPI 3.0 計算區塊
            # (calculate_fss, perform_internal_benchmarking, calculate_mpi_3_0_and_rate 已刪除)
