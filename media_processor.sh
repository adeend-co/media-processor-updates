...
# ... 其他設定 ...
TARGET_DATE="2026-06-26" # <<< 新增：設定您的目標日期
...
display_countdown() {
    ...
    if [ "$remaining_seconds" -le 0 ]; then
        ...
        # 組合顯示訊息 (使用不同顏色區分)
        countdown_message="${CYAN}距離「暑假」（ ${TARGET_DATE} ）尚餘： ${GREEN}${days} ${WHITE}天 ${GREEN}${hours} ${WHITE}時 ${GREEN}${minutes} ${WHITE}分 ${GREEN}${seconds} ${WHITE}秒${RESET}"
    fi
