...
# 腳本設定
SCRIPT_VERSION="v2.5.4-beta.17" # <<< 版本號更新
############################################
# <<< 新增：腳本更新日期 >>>
############################################
...
main() {
    # --- 步驟 1：處理特殊啟動模式 ---
    # 這是自我更新測試的關鍵。當腳本以 --health-check 參數啟動時，
    # 它應該立即成功退出，不做任何其他操作。
...
# --- 執行主函數 ---
# 【關鍵修正】將 "$@" 傳遞給 main 函數。
# "$@" 是一個特殊的 Bash 變數，它包含了所有傳遞給腳本的命令行參數 (例如 --health-check)。
# 這樣可以確保 main 函數能接收到這些參數並做出正確的反應。 [1, 2, 5]
main "$@"
