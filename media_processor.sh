...
# 腳本設定
SCRIPT_VERSION="v2.5.4-beta.11" # <<< 版本號更新
############################################
# <<< 新增：腳本更新日期 >>>
############################################
...
process_single_mp4() {
    ...
    mkdir -p "$DOWNLOAD_PATH"
    if [ ! -w "$DOWNLOAD_PATH" ]; then
        log_message "ERROR" "無法寫入下載目錄：$DOWNLOAD_PATH"; rm -rf "$temp_dir"; return 1
    fi
...
main() {
    ...
    if [[ "$1" == "--health-check" ]]; then
        # 在這個模式下，我們的目標只是驗證腳本可以被 Bash 成功解析和執行到這裡。
        # Bash 在執行這個腳本時，已經從頭到尾讀取並解析了所有的函數定義。
        # 如果有任何致命的語法錯誤，腳本在此之前就已經失敗退出了。
        # 因此，能成功進入這個 if 判斷，就意味著最基本的語法是正確的。
        
        # 我們什麼都不需要做，直接以成功的狀態碼退出即可。
        # 這使得健康檢查幾乎是瞬間完成的。
        exit 0
    fi
    # --- 健康檢查模式結束 ---
