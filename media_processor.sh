...
# 腳本設定
SCRIPT_VERSION="v2.3.6(Experimental)" # <<< 版本號更新
############################################
# <<< 新增：腳本更新日期 >>>
############################################
SCRIPT_UPDATE_DATE="2025-04-10" # 請根據實際情況修改此日期
############################################
...
process_single_mp4_no_normalize_sections() {
    ...
    # --- 格式選擇：最佳視訊 (bv*) 和最佳音訊 (ba*) ---
    local video_format_option="bv*" # Best video overall
    local audio_format_option="ba*" # Best audio overall
    log_message "INFO" "使用格式 (分開下載，bv*/ba*): Video='$video_format_option', Audio='$audio_format_option'"
    ...
    # --- 定義臨時檔案路徑 ---
    local temp_video_raw_base="${temp_dir}/video_section_raw" # 基礎名，yt-dlp 會加副檔名
    local temp_audio_raw_base="${temp_dir}/audio_section_raw" # 基礎名
    local actual_video_raw_file="" # 實際下載的視訊檔
    local actual_audio_raw_file="" # 實際下載的音訊檔
    local temp_audio_aac_file="${temp_dir}/audio_final.aac"   # 轉換後的 AAC 音訊檔

    echo -e "${YELLOW}開始分開下載視訊和音訊指定時段 ($start_time-$end_time)...${RESET}"

    # --- 1. 下載視訊時段 (bv*) ---
    echo -e "${YELLOW}下載視訊流時段 (bv*)...${RESET}"
    local yt_dlp_video_args=(
        yt-dlp
        -f "$video_format_option"
        --download-sections "*${start_time}-${end_time}"
        -o "${temp_video_raw_base}.%(ext)s" # 輸出到基礎名，讓 yt-dlp 決定副檔名
        "$video_url"
        --newline
        # --progress
        --concurrent-fragments "$THREADS"
        --force-keyframes-at-cuts # 嘗試提高切割精度
        --quiet --no-warnings
    )
    log_message "INFO" "執行 yt-dlp (僅視訊時段 bv*): ${yt_dlp_video_args[*]}"
    if ! "${yt_dlp_video_args[@]}" 2> "$temp_dir/yt-dlp-video-section.log"; then
        log_message "ERROR" "視訊時段下載失敗 (bv*)..."
        echo -e "${RED}錯誤：視訊時段下載失敗！${RESET}"
        cat "$temp_dir/yt-dlp-video-section.log"
        [ -d "$temp_dir" ] && rm -rf "$temp_dir"; return 1;
    fi
    # 查找實際下載的視訊檔案
    actual_video_raw_file=$(find "$temp_dir" -maxdepth 1 -name "$(basename "$temp_video_raw_base").*" -print -quit)
    if [ -z "$actual_video_raw_file" ] || [ ! -s "$actual_video_raw_file" ]; then
        log_message "ERROR" "視訊時段下載後檔案不存在或為空 (搜尋基礎名: $temp_video_raw_base.*)"
        echo -e "${RED}錯誤：下載的視訊檔案不存在或為空！${RESET}"
        [ -d "$temp_dir" ] && rm -rf "$temp_dir"; return 1;
    fi
    log_message "INFO" "視訊時段下載完成: $actual_video_raw_file"

    # --- 2. 下載音訊時段 (ba*) ---
    echo -e "${YELLOW}下載音訊流時段 (ba*)...${RESET}"
    local yt_dlp_audio_args=(
        yt-dlp
        -f "$audio_format_option"
        --download-sections "*${start_time}-${end_time}"
        -o "${temp_audio_raw_base}.%(ext)s"
        "$video_url"
        --newline
        # --progress
        --concurrent-fragments "$THREADS"
    ...
    log_message "INFO" "執行 yt-dlp (僅音訊時段 ba*): ${yt_dlp_audio_args[*]}"
    if ! "${yt_dlp_audio_args[@]}" 2> "$temp_dir/yt-dlp-audio-section.log"; then
        log_message "ERROR" "音訊時段下載失敗 (ba*)..."
        echo -e "${RED}錯誤：音訊時段下載失敗！${RESET}"
        cat "$temp_dir/yt-dlp-audio-section.log"
        safe_remove "$actual_video_raw_file" # 清理已下載的視訊
        [ -d "$temp_dir" ] && rm -rf "$temp_dir"; return 1;
    fi
    # 查找實際下載的音訊檔案
    actual_audio_raw_file=$(find "$temp_dir" -maxdepth 1 -name "$(basename "$temp_audio_raw_base").*" -print -quit)
    if [ -z "$actual_audio_raw_file" ] || [ ! -s "$actual_audio_raw_file" ]; then
        log_message "ERROR" "音訊時段下載後檔案不存在或為空 (搜尋基礎名: $temp_audio_raw_base.*)"
    ...
    # --- 3. 轉換音訊為 AAC (邏輯不變) ---
    echo -e "${YELLOW}正在轉換音訊格式為 AAC...${RESET}"
    local ffmpeg_audio_convert_args=(
        ffmpeg -y -i "$actual_audio_raw_file" -vn
        -c:a aac -b:a 256k -ar 44100
        "$temp_audio_aac_file"
    )
    log_message "INFO" "執行 FFmpeg 音訊轉換: ${ffmpeg_audio_convert_args[*]}"
    if ! "${ffmpeg_audio_convert_args[@]}" 2> "$temp_dir/ffmpeg-audio-convert.log"; then
        ...
        safe_remove "$actual_video_raw_file" "$actual_audio_raw_file"
        [ -d "$temp_dir" ] && rm -rf "$temp_dir"; return 1;
    fi
    ...
    log_message "SUCCESS" "音訊成功轉換為 AAC: $temp_audio_aac_file"
    ...
    if ! "${yt_dlp_sub_args[@]}" 2> "$temp_dir/yt-dlp-sections-subs.log"; then
        log_message "WARNING" "下載字幕失敗或無字幕。"; echo -e "${YELLOW}警告：下載字幕失敗或影片無字幕。${RESET}"
    fi
    ...
    local ffmpeg_mux_args=(
        ffmpeg -y
        -i "$actual_video_raw_file"      # 輸入 0: 原始視訊流
        -i "$temp_audio_aac_file"        # 輸入 1: 轉換後的 AAC 音訊流
    )
    local sub_input_start_index=2
    ...
    # 映射流和編碼器
    ffmpeg_mux_args+=(
        "-map" "0:v:0" # 映射輸入 0 的視訊
        "-map" "1:a:0" # 映射輸入 1 的音訊
        "-c:v" "copy" # 嘗試直接複製視訊流 (如果容器不相容 MP4 可能需要 re-encode)
    ...
    for ((i=0; i<${#subtitle_files[@]}; i++)); do
        ...
        ffmpeg_mux_args+=("-metadata:s:s:$sub_stream_index" "language=$ffmpeg_lang")
        ((sub_stream_index++))
    done
    ...
    # 輸出設定
    ffmpeg_mux_args+=("-movflags" "+faststart" "$output_video_file")

    log_message "INFO" "執行 FFmpeg 最終合併 (分開下載源): ${ffmpeg_mux_args[*]}"
    ...
    safe_remove "$actual_video_raw_file" # 清理原始視訊檔
    safe_remove "$actual_audio_raw_file" # 清理原始音訊檔
    safe_remove "$temp_audio_aac_file"   # 清理轉換後的 AAC
    ...
    if [ $result -eq 0 ]; then
        ...
        echo -e "${GREEN}處理完成！影片已儲存至：$output_video_file${RESET}"
        log_message "SUCCESS" "處理完成！影片：$output_video_file"
    else
        echo -e "${RED}處理失敗！${RESET}"
        log_message "ERROR" "處理失敗：$video_url"
    fi
    return $result
}
